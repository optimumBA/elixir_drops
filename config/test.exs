...
# Disable gzip asset serving in test so Plug.Static serves the freshly-built
# plain app.js instead of any stale app.js.gz on disk. The .gz is only
# regenerated by `phx.digest` (assets.deploy), never by `assets.build`, so a
# leftover .gz from a prior branch would otherwise shadow every source edit.
