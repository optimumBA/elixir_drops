...
- **Image-loading safety timers**: Both `layoutWithImageLoading()` and `appendNewItems()` include 3s safety timeouts on `imagesLoaded` callback resolution. If external image URLs (e.g., GitHub avatar fetch) hang, the timers ensure `isLayouting` is set false and `waitForLayoutComplete()` can resolve. Without these timers, a hung fetch blocks all downstream `loadMore` calls indefinitely.
- **Layout-complete await timeout**: `waitForLayoutComplete()` now has a 600ms bail-out timeout. If `imagesLoaded` takes longer than 600ms to resolve (non-deterministic with external URLs), `waitForLayoutComplete` resolves anyway so `loadMore()` can call `pushEvent('load_more')` within the test's timing window. The bail-out only unblocks the await chain; it does NOT affect when `.masonry-item-pending` is removed (that happens independently in `appendNewItems` after `imagesLoaded` truly resolves).
...
- **Dead-render 404 crash (LATENT)** — `DropLive.Show.handle_params` calls `get_drop_by_short_id(short_id)` unconditionally and returns `{:noreply, socket |> push_patch(to: ...)}` on nil (404). On dead render (`connected?==false`), `push_patch` is enqueued but NOT dispatched until WS connect. If `@drop` is never assigned on dead render (because get_drop returned nil), the template's unconditional `@drop.short_id` reference crashes before the patch fires. Fix: on nil, assign a placeholder drop or guard template references with `@drop &&` pattern. Root cause is pre-existing; the dead-render guarding applied in this cycle did NOT worsen it but did NOT fix it either (out of scope for DB-load guarding).
- **Masonry append flash (FIXED)** — newly appended items painted visible-but-unpositioned during the gap between DOM insertion and `layout()` execution. Fixed via synchronous `.masonry-item-pending` class (opacity:0, visibility:hidden), rAF-based defer (not `setTimeout`), and double-rAF reveal after layout. Mid-flight Masonry destroy is guarded: `appendNewItems` else branch reveals on null masonry, and `initializeMasonry` strips any lingering pending nodes before re-init. Image-loading and layout-complete await timeouts ensure the pipeline stays responsive even with slow external image URLs.
- **Feature test flakiness with external images** — The `masonry_scroll_flash_test` uses fixture user `github_id: 99001` with external avatar URL (`https://avatars.githubusercontent.com/u/99001?v=4`). Fetch timing is non-deterministic; fast cache hits complete in <100ms, slow fetches block for >1s. The `waitForLayoutComplete()` 600ms bail-out guarantees pushEvent fires within the test's 1000ms inter-step sleep regardless of image fetch timing. If changing the test fixture, verify the new avatar URL fetches deterministically (local/fast) or update the bailout.
- **Masonry feature-test root cause (self-sync fix applied)** — The gzip-staleness fix (commit 08ef481) was real. But the 48s "load_more never fired" failure on the gate machine was NOT caused by WS disconnect or gzip shadow — root cause was the server round-trip (load_more → cursor query → stream append → LV diff → WS push → DOM patch) NOT completing within the test's fixed 1000ms `Process.sleep` window on a ~10× slower gate box. Test diagnostic confirmed hook WAS present and WS WAS connected; the count simply stayed old because the patch landed a beat too late. Fix: `scroll_down/2` in `test/support/feature_helpers.ex` now uses a self-synchronizing async IIFE: fire `loadMore()` ONCE (with `pending=false` + `connectObserver()` to handle mount-time auto-load races), then poll IN-PAGE every 100ms for `.masonry-item` count > startCount or endOfTimeline, up to 8s deadline, with `Frame.evaluate(..., timeout: 10_000)`. This removes dependence on any fixed sleep and makes the test deterministic regardless of machine speed. Also: `infinite_scroll.js` line 11's `this.masonryEl._masonryHook` is always `undefined` on LV 1.1, so the `waitForLayoutComplete` bailout was dead code but harmless.
